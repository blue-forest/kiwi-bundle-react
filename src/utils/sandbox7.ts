...
type StoreBind<
  ...
  > = {
    ...
    bind: {
      [id in Id]: {
        get: {
          [value in Bindings extends (infer J)[] ? J : never]: Values[value]
        }
        set: {
...
function importStore<Stores extends ImportType>(...s: Stores) {
  return ({} as any) as Stores extends ImportType<infer Bind>
    ? Bind["bind"] /*{ [name in Bind["id"]]: Stores }*/
    : never
}
...
type test3TypeType = { id: number; bind: { [n: number]: string } }
type test3Type<Content extends test3TypeType = test3TypeType> = Content[]
const test3 = <P extends test3Type>(
  ...p: P
): P extends test3Type<infer C> ? C["bind"] : never => ({} as any)
const test4 = test3(
  { id: 1, bind: { 1: "b1" } } as const,
  { id: 2, bind: { 2: "b2" } } as const,
)
...
type input<
  ...
  id extends number = any,
  value extends string = string
  > = { [index in i]: { id: id; value: value } }
const convert = <I extends input>(
  i: I,
): I extends input<infer Id, infer Value> ? { [id in Id]: Value } : never =>
  ({} as any)
